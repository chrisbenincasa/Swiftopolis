...
class ToolButtonPanel: SKNode {
    ...
    override init() {
        ...
        cachedTextures = CachedTextureAtlas.vend("Icons") {
            println("-- preloaded icon textures --")
            self.initButtons()
        }
    }
    
    private func initButtons() {
        var baseline: CGPoint = CGPoint.zeroPoint
        
        for (idx, tool) in enumerate([Tool.Bulldozer, Tool.Wire, Tool.Park]) {
        ...
        for (idx, tool) in enumerate([Tool.Road, Tool.Rail]) {
        ...
        for (idx, tool) in enumerate([Tool.Residential, Tool.Commercial, Tool.Industrial]) {
        ...
        for (idx, tool) in enumerate([Tool.FireStation, Tool.Query, Tool.PoliceStation]) {
            makeButton(tool, size: ToolButtonHelpers.SMALL_BUTTON_SIZE, offset: idx, baseline: baseline)
        }
        ...
        for (idx, tool) in enumerate([Tool.Coal, Tool.Nuclear]) {
            makeButton(tool, size: ToolButtonHelpers.MEDIUM_BUTTON_SIZE, offset: idx, baseline: baseline)
        }
        
        baseline.y -= ToolButtonHelpers.MEDIUM_BUTTON_SIZE.height + SPACE_BETWEEN_BUTTONS
        
        for (idx, tool) in enumerate([Tool.Stadium, Tool.Seaport]) {
            makeButton(tool, size: ToolButtonHelpers.MEDIUM_BUTTON_SIZE, offset: idx, baseline: baseline)
        }
