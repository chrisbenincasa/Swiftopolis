...
class MapGenerator {
    ...
    private func makeNakedIsland() {
        ...
        for x in 0.stride(to: width - 5, by: 2) {
            currentX = x
        ...
        for y in 0.stride(to: height - 5, by: 2) {
            currentY = y
    ...
    private func treeSplash(x x: Int, y: Int) {
        ...
        for _ in 0...dis - 1 {
            var dir = Int(arc4random_uniform(8))
            moveMap(&dir)
    ...
    private func moveMap(inout dir: Int) {
        dir &= 7
