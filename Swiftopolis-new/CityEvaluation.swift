...
class CityEvaluation {
    ...
    private func doCityProblems() {
        ...
        var sortedKeys = Array(problemScores.keys).sort({ [unowned self] in self.problemVotes[$0] >= self.problemVotes[$1] })
    ...
    private func calculateUnemployment() -> Int {
        ...
        let jobs = Double(city.census.residentialPopulation) / Double(availableJobs)
    ...
    private func clamp<T : Comparable>(value value: T, minimum: T, maximum: T) -> T {
