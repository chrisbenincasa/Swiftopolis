...
//            println("Long simulation step occurred \(phase): \(timeInterval) seconds");
    ...
    private func mapScanTile(x x: Int, y: Int) {
    ...
    private func takeCensus(historical historical: Bool) {
        var resMax = 0, comMax = 0, indMax = 0
        let (start, end) = historical ? (238, 120) : (118, 0)
        
        for i in start.stride(through: end, by: -1) {
            if !historical {
        ...
        history.shiftResidential(census.residentialPopulation / 8, atIndex: end)
        history.shiftCommercial(census.commercialPopulation, atIndex: end)
        history.shiftIndustrial(census.industrialPopulation, atIndex: end)
        
        if !historical {
            history.cityTime = cityTime
            
            census.crimeRamp += (census.crimeAverage - census.crimeRamp) / 4
            history.shiftCrime(min(255, census.crimeRamp))
            
            census.pollutionRamp += (census.pollutionAverage - census.pollutionRamp) / 4
            history.shiftPollution(min(255, census.pollutionRamp))
            ...
            history.shiftMoney(moneyScaled)
            ...
            history.shiftPollution(history.pollution[0], atIndex: end)
            history.shiftCrime(history.crime[0], atIndex: end)
            history.shiftMoney(history.money[0], atIndex: end)
        }
    }
    ...
    private func collectTax(flush: Bool) {
        let budgetNumbers = generateBudgetNumbers(roadTotal: census.roadTotal, railTotal: census.railTotal, totalPopulation: census.totalPopulation, fireStationCount: census.fireStationCount, policeStationCount: census.policeCount)
...
//        println("smoothTerrain took \(timeInterval) seconds")
    ...
    private func computePopulationDensity(x x: Int, y: Int, tile: UInt16) -> Int {
    ...
    private func onTileChanged(x x: Int, y: Int) {
        for subscriber in self.subscribers {
    ...
    private func generateBudgetNumbers(roadTotal roadTotal: Int, railTotal: Int, totalPopulation: Int, fireStationCount: Int, policeStationCount: Int) -> BudgetNumbers {
