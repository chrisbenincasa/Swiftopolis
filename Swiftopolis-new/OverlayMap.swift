...
class OverlayMap: SKNode, Subscriber {
    ...
    required init(engine _engine: Engine, connectedMap _connectedMap: CityWorld) {
        ...
        tileImages = TileImages.vend(3) {
            print("-- preloaded 3px tile textures --")
        }
        userInteractionEnabled = true
        
        viewportNode = SKShapeNode(rect: CGRect(origin: CGPoint.zero, size: connectedMap.viewportSize * TILE_SIZE))
...
//        println("-- redrawing overlay map \(NSDate())--")
        ...
        dispatch_async(self.barrier) {
            ...
            for y in 0 ..< self.engine.city.map.height {
                for x in 0 ..< self.engine.city.map.width {
                    ...
                    image.drawAtPoint(position, fromRect: NSRect.zero, operation: .CompositeSourceOver, fraction: 1.0)
                }
            }
    ...
    private func moveViewTo(point: NSPoint) {
        var p = NSPoint(x: point.x, y: point.y)
        ...
        let normalized = PointUtils.normalizeMapPoint(&p, bounds: citySize, viewport: connectedViewport)
    ...
    private lazy var debouncedDraw: () -> () = debounce(1.0, action: self.draw)
    private lazy var throttledDraw: () -> () = throttle(1.0, action: self.draw)
